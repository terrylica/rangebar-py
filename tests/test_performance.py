...
class TestThroughputBenchmarks:
    ...
    def test_throughput_1k_trades(self, benchmark):
        ...
        result = benchmark(
            process_trades_to_dataframe, trades, threshold_decimal_bps=250
        )
        assert len(result) > 0
    ...
    def test_throughput_1m_trades(self, benchmark):
        ...
        trades = [
            {
                "timestamp": 1704067200000 + i * 10,
                "price": 42000.0 + i * 0.001,
                "quantity": 1.0,
            }
...
class TestMemoryBenchmarks:
    ...
    def test_memory_1m_trades(self):
        ...
        # Process trades
        result = process_trades_to_dataframe(trades, threshold_decimal_bps=250)
...
class TestCompressionRatios:
    ...
    @pytest.mark.parametrize(
        ("threshold_decimal_bps", "expected_min_bars"),
        [
    ...
    def test_compression_ratio(self, threshold_decimal_bps, expected_min_bars):
        ...
        result = process_trades_to_dataframe(
            trades, threshold_decimal_bps=threshold_decimal_bps
        )

        # Validate bar count is reasonable
        assert len(result) >= expected_min_bars, (
            f"Threshold {threshold_decimal_bps}bps: {len(result)} bars "
            f"< expected minimum {expected_min_bars}"
        )
    ...
    def test_compression_monotonicity(self):
        ...
        for threshold in thresholds:
            result = process_trades_to_dataframe(
                trades, threshold_decimal_bps=threshold
            )
            bar_counts[threshold] = len(result)

        # Higher threshold = fewer bars (more compression)
        assert (
            bar_counts[100] >= bar_counts[250] >= bar_counts[500] >= bar_counts[1000]
