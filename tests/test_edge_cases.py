...
class TestErrorHandling:
    ...
    def test_dataframe_with_both_quantity_and_volume(self):
        ...
        result = process_trades_to_dataframe(df, threshold_decimal_bps=250)
    ...
    def test_dataframe_missing_both_quantity_and_volume(self):
        ...
        with pytest.raises((ValueError, KeyError)):
            process_trades_to_dataframe(df, threshold_decimal_bps=250)

    def test_extremely_large_threshold(self):
        """Test threshold at upper boundary."""
        processor = RangeBarProcessor(threshold_decimal_bps=100_000)  # 100% threshold
    ...
    def test_empty_trades_list(self):
        ...
        df = process_trades_to_dataframe([], threshold_decimal_bps=250)
        assert len(df) == 0
        ...
        df = pd.DataFrame(
            {
                "timestamp": pd.date_range(
                    "2024-01-01", periods=100, freq="min", tz="UTC"
                ),
