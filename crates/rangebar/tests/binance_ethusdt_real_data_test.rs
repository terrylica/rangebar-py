//! Binance ETHUSDT Real Data Integration Tests
//!
//! Validates range bar construction using real ETHUSDT market data from CSV.
//! Tests multiple threshold scenarios and validates data integrity.

use rangebar::{FixedPoint, RangeBar, RangeBarProcessor};
use rangebar_core::test_data_loader::load_ethusdt_test_data;

#[test]
fn test_ethusdt_data_integrity() {
    // Validate real ETHUSDT CSV data integrity
    let trades = load_ethusdt_test_data().expect("Failed to load ETHUSDT test data");

    assert_eq!(trades.len(), 10000, "Expected 10,000 ETHUSDT trades");
...
#[test]
fn test_ethusdt_standard_threshold() {
    ...
    let range_bars = processor
        ...
        .expect("Failed to process ETHUSDT trades");
    ...
    println!(
        "ETHUSDT (25 dbps): {} trades → {} range bars",
        trades.len(),
        range_bars.len()
    );
...
#[test]
fn test_ethusdt_medium_threshold() {
    ...
    println!(
        "ETHUSDT (50 dbps): {} trades → {} range bars",
        trades.len(),
        range_bars.len()
    );
...
#[test]
fn test_ethusdt_wide_threshold() {
    ...
    println!(
        "ETHUSDT (100 dbps): {} trades → {} range bars",
        trades.len(),
        range_bars.len()
    );
...
#[test]
fn test_ethusdt_threshold_scaling() {
    ...
    println!("ETHUSDT threshold scaling:");
