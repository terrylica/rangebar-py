...
"""Multi-threshold combinations within market regimes (Polars).

Combine two successful approaches:
1. Market regime filtering (SMA/RSI)
2. Multi-threshold combinations (50|100|200 dbps)

Hypothesis: Multi-threshold combinations may show stronger signals
when filtered by market regime.
...
def run_regime_multithreshold_analysis(
    symbol: str,
    start_date: str,
    end_date: str,
    horizon: int = 10,
    ...
    """Run multi-threshold analysis within market regimes."""
    log_info("Starting regime + multi-threshold analysis", symbol=symbol, horizon=horizon)
    ...
    # Process 100 dbps with regime classification
    ...
    df_100_c = df_100.select(["timestamp", "Close", "pattern_2bar", "regime", f"fwd_ret_{horizon}"]).collect()
    ...
    # Create multi-threshold combination code
    ...
    clean_df = merged.drop_nulls(subset=["combo", "regime", return_col])

    regime_results = check_odd_robustness_by_regime(
        clean_df,
        "combo",
        return_col,
    ...
    # Count robust patterns per regime
    total_robust = 0
    for regime, patterns in regime_results.items():
        robust_count = len(patterns)
        total_robust += robust_count
        log_info(f"Regime {regime}", symbol=symbol, regime=regime, robust_count=robust_count)
...
def main() -> int:
    """Main entry point."""
    log_info(
        "Starting multi-threshold + regime combination analysis (Polars)",
        script="multithreshold_regime_combinations_polars.py",
    )
    ...
    end_date = "2026-01-31"
    horizon = 10
    min_samples = 50  # Lower threshold for regime-specific analysis
    min_t_stat = 5.0

    log_info("=== MULTI-THRESHOLD + REGIME COMBINATION ANALYSIS ===")

    all_results = {}

    for symbol in symbols:
        try:
            result = run_regime_multithreshold_analysis(
                symbol=symbol,
                start_date=start_date,
                end_date=end_date,
                horizon=horizon,
    ...
    log_info("=== CROSS-SYMBOL REGIME + MULTI-THRESHOLD SUMMARY ===")

    # Collect all regime|pattern combinations and count universal ones
    pattern_counts = Counter()
    for _symbol, result in all_results.items():
        for _regime, patterns in result.get("by_regime", {}).items():
    ...
    log_info(
        "Universal patterns (regime + multi-threshold)",
    ...
    for pattern in universal:
        regime = pattern.split("|")[0]
        if regime not in regime_universal:
            regime_universal[regime] = []
        regime_universal[regime].append(pattern)

    for regime, patterns in sorted(regime_universal.items()):
        log_info(
            f"Universal in {regime}",
            regime=regime,
            count=len(patterns),
    ...
    log_info(
        ...
        regimes_with_universal=len(regime_universal),
    )

    # Save results
    output_path = Path("/tmp/multithreshold_regime_combinations_polars.json")
    with output_path.open("w") as f:
        json.dump({
            "all_results": all_results,
            "universal_patterns": universal,
            "by_regime": regime_universal,
        }, f, indent=2, default=str)
    log_info("Results saved", path=str(output_path))

    log_info("Multi-threshold + regime combination analysis complete")
    return 0
